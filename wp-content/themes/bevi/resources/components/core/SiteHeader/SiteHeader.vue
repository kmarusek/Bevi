    ...
    <div class="px-4 md:px-10">
      <nav class="nav">
        <div class="logo-container">
          <a href="/">
            <img
              :src="require('~/assets/images/bevi.svg').default"
              alt="Bevi Logo"
              class="w-20"
            >
          </a>
        ...
        <button
          ...
          aria-haspopup="true"
          aria-controls="menu"
          :aria-label="isMobileNavOpen ? 'Navigation open' : 'Navigation closed'"
        >
          <span :aria-label="isMobileNavOpen ? 'Navigation open' : 'Navigation closed'" />
          <span :aria-label="isMobileNavOpen ? 'Navigation open' : 'Navigation closed'" />
          <span :aria-label="isMobileNavOpen ? 'Navigation open' : 'Navigation closed'" />
        </button>
      </nav>
  ...
  export default {
    ...
    methods: {
      toggleMobileNav() {
        ...
        // Trapping focus on menu for accessibility
        const footer = document.querySelector('footer');
        const main = document.querySelector('main');

        footer.hidden = this.isMobileNavOpen;
        main.hidden = this.isMobileNavOpen;
      },
      startAnimation() {
...
.header {
  ...
  .mobile-nav-item {
    @apply absolute flex flex-col justify-center content-around flex-wrap w-full inset-0;
    z-index: -1;
  }
}
</style>
