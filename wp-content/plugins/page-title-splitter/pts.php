...
function pt_splitter_custom_script() {
	...
	wp_register_style('pt_splitter_css', plugins_url( 'css/pts.css?ver=2.0.9', __FILE__ ));
	wp_enqueue_style('pt_splitter_css');
	wp_enqueue_script('pt_splitter_js', plugins_url( 'js/pts.js?ver=2.0.9', __FILE__ ), array('jquery'));
}
...
function pt_splitter_widget($instance, $obj, $args) {
	...
	$_classes = (get_option('pt_splitter_options')) ? get_option('pt_splitter_options') : $pt_splitter_defaults;
	if(array_key_exists('cssbox_widget', $_classes) && $_classes['cssbox_widget'] === 'no') {
...
if(in_array( 'woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')) ) ) {
  require_once('class/pts-woocommerce.php');
}
?>