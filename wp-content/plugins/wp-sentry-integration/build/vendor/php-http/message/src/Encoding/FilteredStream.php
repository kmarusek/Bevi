...
use WPSentry\ScopedVendor\Http\Message\Decorator\StreamDecorator;
 ...
 */
abstract class FilteredStream implements \WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface
{
    use StreamDecorator {
     ...
     */
    public function __construct(\WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface $stream, $readFilterOptions = null, $writeFilterOptions = null)
    {
        if (null !== $readFilterOptions) {
            $this->readFilterCallback = \WPSentry\ScopedVendor\Clue\StreamFilter\fun($this->readFilter(), $readFilterOptions);
        } else {
            $this->readFilterCallback = \WPSentry\ScopedVendor\Clue\StreamFilter\fun($this->readFilter());
        }
        if (null !== $writeFilterOptions) {
            $this->writeFilterCallback = \WPSentry\ScopedVendor\Clue\StreamFilter\fun($this->writeFilter(), $writeFilterOptions);
            ...
            $this->writeFilterCallback = \WPSentry\ScopedVendor\Clue\StreamFilter\fun($this->writeFilter());
        }
        $this->stream = $stream;
    }
    /**
