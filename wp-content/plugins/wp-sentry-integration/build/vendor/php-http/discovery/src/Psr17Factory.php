...
class Psr17Factory implements \WPSentry\ScopedVendor\Psr\Http\Message\RequestFactoryInterface, \WPSentry\ScopedVendor\Psr\Http\Message\ResponseFactoryInterface, \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestFactoryInterface, \WPSentry\ScopedVendor\Psr\Http\Message\StreamFactoryInterface, \WPSentry\ScopedVendor\Psr\Http\Message\UploadedFileFactoryInterface, \WPSentry\ScopedVendor\Psr\Http\Message\UriFactoryInterface
{
    ...
    {
        $factory = $this->requestFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findRequestFactory());
    ...
    {
        $factory = $this->responseFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findResponseFactory());
    ...
    {
        $factory = $this->serverRequestFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findServerRequestFactory());
        return $factory->createServerRequest(...\func_get_args());
    }
    public function createServerRequestFromGlobals(array $server = null, array $get = null, array $post = null, array $cookie = null, array $files = null, \WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface $body = null) : \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface
    {
    ...
    {
        $factory = $this->streamFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findStreamFactory());
    ...
    {
        $factory = $this->uploadedFileFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findUploadedFileFactory());
    ...
    {
        $factory = $this->uriFactory ?? $this->setFactory(\WPSentry\ScopedVendor\Http\Discovery\Psr17FactoryDiscovery::findUriFactory());
        return $factory->createUri(...\func_get_args());
    }
    public function createUriFromGlobals(array $server = null) : \WPSentry\ScopedVendor\Psr\Http\Message\UriInterface
    {
        return $this->buildUriFromGlobals($this->createUri(''), $server ?? $_SERVER);
    }
    private function setFactory($factory)
    {
        if (!$this->requestFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\RequestFactoryInterface) {
            $this->requestFactory = $factory;
        }
        if (!$this->responseFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\ResponseFactoryInterface) {
            $this->responseFactory = $factory;
        }
        if (!$this->serverRequestFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestFactoryInterface) {
            $this->serverRequestFactory = $factory;
        }
        if (!$this->streamFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\StreamFactoryInterface) {
            $this->streamFactory = $factory;
        }
        if (!$this->uploadedFileFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\UploadedFileFactoryInterface) {
            $this->uploadedFileFactory = $factory;
        }
        if (!$this->uriFactory && $factory instanceof \WPSentry\ScopedVendor\Psr\Http\Message\UriFactoryInterface) {
            $this->uriFactory = $factory;
        }
        return $factory;
    }
    private function buildServerRequestFromGlobals(\WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface $request, array $server, array $files) : \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface
    {
    ...
    private function buildUriFromGlobals(\WPSentry\ScopedVendor\Psr\Http\Message\UriInterface $uri, array $server) : \WPSentry\ScopedVendor\Psr\Http\Message\UriInterface
    {
