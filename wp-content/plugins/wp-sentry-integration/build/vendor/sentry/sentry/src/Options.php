...
final class Options
{
    ...
    {
        $this->resolver = new \WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\OptionsResolver();
        $this->configureOptions($this->resolver);
        $this->options = $this->resolver->resolve($options);
        if (\true === $this->options['enable_tracing'] && null === $this->options['traces_sample_rate']) {
    ...
    {
        if (0 === \func_num_args() || \false !== \func_get_arg(0)) {
            @\trigger_error(\sprintf('Method %s() is deprecated since version 3.2 and will be removed in 4.0.', __METHOD__), \E_USER_DEPRECATED);
        }
     ...
     */
    public function getDsn() : ?\Sentry\Dsn
    {
    ...
    {
        @\trigger_error(\sprintf('Method %s() is deprecated since version 3.2 and will be removed in 4.0. Use Sentry\\Scope::setTags() instead.', __METHOD__), \E_USER_DEPRECATED);
    ...
    {
        $resolver->setDefaults(['integrations' => [], 'default_integrations' => \true, 'send_attempts' => 0, 'prefixes' => \array_filter(\explode(\PATH_SEPARATOR, \get_include_path() ?: '')), 'sample_rate' => 1, 'enable_tracing' => null, 'traces_sample_rate' => null, 'traces_sampler' => null, 'profiles_sample_rate' => null, 'attach_stacktrace' => \false, 'context_lines' => 5, 'enable_compression' => \true, 'environment' => $_SERVER['SENTRY_ENVIRONMENT'] ?? null, 'logger' => 'php', 'release' => $_SERVER['SENTRY_RELEASE'] ?? null, 'dsn' => $_SERVER['SENTRY_DSN'] ?? null, 'server_name' => \gethostname(), 'ignore_exceptions' => [], 'ignore_transactions' => [], 'before_send' => static function (\Sentry\Event $event) : Event {
            return $event;
        ...
        $resolver->setNormalizer('tags', static function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, array $value) : array {
            if (!empty($value)) {
                @\trigger_error('The option "tags" is deprecated since version 3.2 and will be removed in 4.0. Either set the tags on the scope or on the event.', \E_USER_DEPRECATED);
            }
            return $value;
        });
        $resolver->setNormalizer('prefixes', function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, array $value) {
            return \array_map([$this, 'normalizeAbsolutePath'], $value);
        });
        $resolver->setNormalizer('in_app_exclude', function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, array $value) {
            return \array_map([$this, 'normalizeAbsolutePath'], $value);
        });
        $resolver->setNormalizer('in_app_include', function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, array $value) {
            return \array_map([$this, 'normalizeAbsolutePath'], $value);
        });
        $resolver->setNormalizer('logger', function (\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, ?string $value) : ?string {
            if ('php' !== $value) {
                @\trigger_error('The option "logger" is deprecated.', \E_USER_DEPRECATED);
            }
            return $value;
        });
    }
     ...
     */
    private function normalizeDsnOption(\WPSentry\ScopedVendor\Symfony\Component\OptionsResolver\Options $options, $value) : ?\Sentry\Dsn
    {
        if (null === $value || \is_bool($value)) {
            return null;
        }
        if ($value instanceof \Sentry\Dsn) {
            return $value;
        }
        ...
        return \Sentry\Dsn::createFromString($value);
    }
    /**
    ...
    {
        if (null === $dsn || $dsn instanceof \Sentry\Dsn) {
            return \true;
        }
        if (\is_bool($dsn)) {
            return \false === $dsn;
