...
final class FrameBuilder
{
     ...
     */
    public function __construct(\Sentry\Options $options, \Sentry\Serializer\RepresentationSerializerInterface $representationSerializer)
    {
     ...
     */
    public function buildFromBacktraceFrame(string $file, int $line, array $backtraceFrame) : \Sentry\Frame
    {
        ...
        if (isset($backtraceFrame['class']) && isset($backtraceFrame['function'])) {
            ...
            if (\str_starts_with($functionName, \Sentry\Frame::ANONYMOUS_CLASS_PREFIX)) {
                $functionName = \Sentry\Frame::ANONYMOUS_CLASS_PREFIX . $this->stripPrefixFromFilePath(\substr($backtraceFrame['class'], \strlen(\Sentry\Frame::ANONYMOUS_CLASS_PREFIX)));
            }
        ...
        return new \Sentry\Frame($functionName, $strippedFilePath, $line, $rawFunctionName, \Sentry\Frame::INTERNAL_FRAME_FILENAME !== $file ? $file : null, $this->getFunctionArguments($backtraceFrame), $this->isFrameInApp($file, $functionName));
    }
    /**
    ...
    {
        if (\Sentry\Frame::INTERNAL_FRAME_FILENAME === $file) {
            return \false;
        }
