...
final class DynamicSamplingContext
{
    ...
    public static function fromHeader(string $header) : self
    {
        ...
        foreach (\explode(',', $header) as $listMember) {
            if (empty(\trim($listMember))) {
                continue;
            }
            $keyValueAndProperties = \explode(';', $listMember, 2);
            $keyValue = \trim($keyValueAndProperties[0]);
     ...
     */
    public static function fromTransaction(\Sentry\Tracing\Transaction $transaction, \Sentry\State\HubInterface $hub) : self
    {
        $samplingContext = new self();
        ...
        $hub->configureScope(static function (\Sentry\State\Scope $scope) use($samplingContext) : void {
