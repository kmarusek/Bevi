 ...
 */
final class RequestIntegration implements \Sentry\Integration\IntegrationInterface
{
    /**
    ...
    {
        \Sentry\State\Scope::addGlobalEventProcessor(function (\Sentry\Event $event) : Event {
    ...
    private function processEvent(\Sentry\Event $event, \Sentry\Options $options) : void
    {
        $request = $this->requestFetcher->fetchRequest();
        if (null === $request) {
            return;
        }
        $requestData = ['url' => (string) $request->getUri(), 'method' => $request->getMethod()];
        ...
        if ($options->shouldSendDefaultPii()) {
            ...
            if (isset($serverParams['REMOTE_ADDR'])) {
                ...
                if (null === $user) {
                    $user = \Sentry\UserDataBag::createFromUserIpAddress($serverParams['REMOTE_ADDR']);
     ...
     * @return mixed
     */
    private function captureRequestBody(\Sentry\Options $options, \WPSentry\ScopedVendor\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        if ('application/json' === $request->getHeaderLine('Content-Type')) {
            ...
            } catch (\Sentry\Exception\JsonException $exception) {
    ...
    private function parseUploadedFiles(array $uploadedFiles) : array
    {
        ...
        foreach ($uploadedFiles as $key => $item) {
            if ($item instanceof \WPSentry\ScopedVendor\Psr\Http\Message\UploadedFileInterface) {
                $result[$key] = ['client_filename' => $item->getClientFilename(), 'client_media_type' => $item->getClientMediaType(), 'size' => $item->getSize()];
            } elseif (\is_array($item)) {
                $result[$key] = $this->parseUploadedFiles($item);
            } else {
                throw new \UnexpectedValueException(\sprintf('Expected either an object implementing the "%s" interface or an array. Got: "%s".', \WPSentry\ScopedVendor\Psr\Http\Message\UploadedFileInterface::class, \is_object($item) ? \get_class($item) : \gettype($item)));
            }
        }
        return $result;
    }
