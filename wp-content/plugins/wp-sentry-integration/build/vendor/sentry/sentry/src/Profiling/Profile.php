...
final class Profile
{
    ...
    public function setEventId(\Sentry\EventId $eventId) : void
    {
     ...
     */
    public function getFormattedData(\Sentry\Event $event) : ?array
    {
        ...
        foreach ($loggedStacks as $stackId => $stack) {
            foreach ($stack['trace'] as $frame) {
                ...
                $frames[] = ['filename' => $file, 'abs_path' => $absolutePath, 'module' => $module, 'function' => $function, 'lineno' => !empty($frame['line']) ? (int) $frame['line'] : null];
                $stacks[$stackId][] = $frameIndex;
                ++$frameIndex;
            }
            $duration = $stack['timestamp'];
            $samples[] = ['elapsed_since_start_ns' => (int) \round($duration * 1000000000.0), 'stack_id' => $stackId, 'thread_id' => self::THREAD_ID];
        }
        ...
        return ['device' => ['architecture' => $osContext->getMachineType()], 'event_id' => $this->eventId ? (string) $this->eventId : \Sentry\Util\SentryUid::generate(), 'os' => ['name' => $osContext->getName(), 'version' => $osContext->getVersion(), 'build_number' => $osContext->getBuild() ?? ''], 'platform' => 'php', 'release' => $event->getRelease() ?? '', 'environment' => $event->getEnvironment() ?? \Sentry\Event::DEFAULT_ENVIRONMENT, 'runtime' => ['name' => $runtimeContext->getName(), 'version' => $runtimeContext->getVersion()], 'timestamp' => $startTime->format(\DATE_RFC3339_EXTENDED), 'transaction' => ['id' => (string) $event->getId(), 'name' => $event->getTransaction(), 'trace_id' => $event->getTraceId(), 'active_thread_id' => self::THREAD_ID], 'version' => self::VERSION, 'profile' => ['frames' => $frames, 'samples' => $samples, 'stacks' => $stacks]];
    }
    /**
    ...
    private function prepareStacks() : array
    {
        ...
        foreach ($this->excimerLog as $stack) {
            if ($stack instanceof \WPSentry\ScopedVendor\ExcimerLogEntry) {
     ...
     */
    private function validateOsContext(?\Sentry\Context\OsContext $osContext) : bool
    {
        if (null === $osContext) {
            return \false;
        }
     ...
     */
    private function validateRuntimeContext(?\Sentry\Context\RuntimeContext $runtimeContext) : bool
    {
     ...
     */
    private function validateEvent(\Sentry\Event $event) : bool
    {
