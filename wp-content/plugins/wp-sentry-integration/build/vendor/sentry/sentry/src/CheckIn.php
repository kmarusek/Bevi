...
final class CheckIn
{
     ...
     */
    public function __construct(string $monitorSlug, \Sentry\CheckInStatus $status, string $id = null, ?string $release = null, ?string $environment = null, $duration = null)
    {
        $this->setMonitorSlug($monitorSlug);
        $this->setStatus($status);
        $this->setId($id ?? \Sentry\Util\SentryUid::generate());
        $this->setRelease($release ?? '');
        $this->setEnvironment($environment ?? \Sentry\Event::DEFAULT_ENVIRONMENT);
    ...
    public function getStatus() : \Sentry\CheckInStatus
    {
        return $this->status;
    }
    public function setStatus(\Sentry\CheckInStatus $status) : void
    {
        $this->status = $status;
    }
