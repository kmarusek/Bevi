...
class Span
{
     ...
     * @internal
     */
    public function __construct(?\Sentry\Tracing\SpanContext $context = null)
    {
        if (null === $context) {
            $this->traceId = \Sentry\Tracing\TraceId::generate();
            $this->spanId = \Sentry\Tracing\SpanId::generate();
            $this->startTimestamp = \microtime(\true);
            return;
        }
        $this->traceId = $context->getTraceId() ?? \Sentry\Tracing\TraceId::generate();
        $this->spanId = $context->getSpanId() ?? \Sentry\Tracing\SpanId::generate();
        $this->startTimestamp = $context->getStartTimestamp() ?? \microtime(\true);
     ...
     */
    public function setSpanId(\Sentry\Tracing\SpanId $spanId) : void
    {
        $this->spanId = $spanId;
    }
    /**
     ...
     */
    public function setTraceId(\Sentry\Tracing\TraceId $traceId) : void
    {
     ...
     */
    public function getParentSpanId() : ?\Sentry\Tracing\SpanId
    {
     ...
     */
    public function setParentSpanId(?\Sentry\Tracing\SpanId $parentSpanId) : void
    {
     ...
     */
    public function getStatus() : ?\Sentry\Tracing\SpanStatus
    {
        return $this->status;
    }
    /**
     ...
     */
    public function setStatus(?\Sentry\Tracing\SpanStatus $status) : void
    {
        $this->status = $status;
    }
    /**
    ...
    public function setHttpStatus(int $statusCode) : void
    {
        ...
        if ($status !== \Sentry\Tracing\SpanStatus::unknownError()) {
            $this->status = $status;
        }
    }
    /**
     ...
     */
    public function startChild(\Sentry\Tracing\SpanContext $context) : self
    {
     ...
     * @internal
     */
    public function getSpanRecorder() : ?\Sentry\Tracing\SpanRecorder
    {
