...
abstract class AbstractSerializer
{
     ...
     */
    public function __construct(\Sentry\Options $options, int $maxDepth = 3, ?string $mbDetectOrder = null)
    {
                ...
                foreach ($classSerializers as $classSerializer) {
                    try {
                        ...
                        if (\is_array($serializedObjectData)) {
                            return ['class' => \get_class($value), 'data' => $this->serializeRecursively($serializedObjectData, $_depth + 1)];
                        }
                    } catch (\Throwable $e) {
    ...
    protected function resolveClassSerializers($object) : array
    {
        ...
        if ($object instanceof \Sentry\Serializer\SerializableInterface) {
            $serializers[] = static function (\Sentry\Serializer\SerializableInterface $object) : ?array {
    ...
    {
        if ($_depth >= $this->maxDepth || \in_array(\spl_object_hash($object), $hashes, \true)) {
    ...
    protected function serializeCallable($callable) : string
    {
        ...
        if (!\is_callable($callable)) {
            throw new \Sentry\Exception\InvalidArgumentException(\sprintf('Expecting callable, got %s', \is_object($callable) ? \get_class($callable) : \gettype($callable)));
        }
        try {
