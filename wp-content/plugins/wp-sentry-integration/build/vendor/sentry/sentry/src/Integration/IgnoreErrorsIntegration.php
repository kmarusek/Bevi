...
final class IgnoreErrorsIntegration implements \Sentry\Integration\IntegrationInterface
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $resolver->setDefaults(['ignore_exceptions' => [], 'ignore_tags' => []]);
    ...
    private function isIgnoredException(\Sentry\Event $event, array $options) : bool
    {
        ...
        foreach ($options['ignore_exceptions'] as $ignoredException) {
            if (\is_a($exceptions[0]->getType(), $ignoredException, \true)) {
                return \true;
            }
        }
        return \false;
     ...
     * @psalm-param IntegrationOptions $options
     */
    private function isIgnoredTag(\Sentry\Event $event, array $options) : bool
    {
        $tags = $event->getTags();
        if (empty($tags)) {
            return \false;
