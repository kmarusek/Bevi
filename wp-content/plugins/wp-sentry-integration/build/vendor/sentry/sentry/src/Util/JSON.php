...
final class JSON
{
    ...
    public static function encode($data, int $options = 0, int $maxDepth = 512) : string
    {
        ...
        if ($encounteredAnyError && ('null' === $encodedData || \false === $encodedData) || !\in_array(\json_last_error(), $allowedErrors, \true)) {
            throw new \Sentry\Exception\JsonException(\sprintf('Could not encode value into JSON format. Error was: "%s".', \json_last_error_msg()));
        }
        return $encodedData;
    }
    /**
    ...
    {
        $decodedData = \json_decode($data, \true);
        if (\JSON_ERROR_NONE !== \json_last_error()) {
            throw new \Sentry\Exception\JsonException(\sprintf('Could not decode value from JSON format. Error was: "%s".', \json_last_error_msg()));
        }
        return $decodedData;
    }
}
