...
final class Scope
{
    ...
    {
        if (!\is_array($user) && !$user instanceof \Sentry\UserDataBag) {
            throw new \TypeError(\sprintf('The $user argument must be either an array or an instance of the "%s" class. Got: "%s".', \Sentry\UserDataBag::class, \get_debug_type($user)));
        }
        if (\is_array($user)) {
            $user = \Sentry\UserDataBag::createFromArray($user);
        }
        if (null === $this->user) {
            $this->user = $user;
        } else {
     ...
     * @return $this
     */
    public function setLevel(?\Sentry\Severity $level) : self
    {
        $this->level = $level;
        return $this;
    }
     ...
     * @return $this
     */
    public function addBreadcrumb(\Sentry\Breadcrumb $breadcrumb, int $maxBreadcrumbs = 100) : self
    {
     ...
     */
    public function applyToEvent(\Sentry\Event $event, ?\Sentry\EventHint $hint = null) : ?\Sentry\Event
    {
        ...
        foreach (\array_merge(self::$globalEventProcessors, $this->eventProcessors) as $processor) {
            ...
            if (!$event instanceof \Sentry\Event) {
                throw new \InvalidArgumentException(\sprintf('The event processor must return null or an instance of the %s class', \Sentry\Event::class));
            }
        }
        return $event;
    }
     ...
     * @return $this
     */
    public function setSpan(?\Sentry\Tracing\Span $span) : self
    {
        $this->span = $span;
        return $this;
    }
