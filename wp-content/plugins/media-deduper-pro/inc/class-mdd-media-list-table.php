...
class MDD_Pro_Media_List_Table extends WP_Media_List_Table {
	...
	public function no_items() {
		switch ( Media_Deduper_Pro::active_tab() ) {
			case 'alt-text':
				_e( 'Great news, no images missing alt text were found!', 'media-deduper' );
				break;
			case 'duplicates':
			default:
	...
	protected function get_bulk_actions() {
		$actions = array();
		if ( 'duplicates' === Media_Deduper_Pro::active_tab() ) {
	...
	public function column_parent( $post ) {
		...
		$stock_detach_url = add_query_arg(
			array(
		...
		// upload.php one.
		$custom_detach_url = add_query_arg(
			array(
				'page' => 'media-deduper',
			), $stock_detach_url
		);
